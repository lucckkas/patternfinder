...
\begin{tikzpicture}[
    ...
    % ========== GENERACIÓN DE JOBS ==========
    \node[mainbox, fill=orange!80!black] (genjobs) {Generar\\pares $(i,j)$};

    % ========== ENTRADA (arriba de genjobs) ==========
    \node[mainbox, fill=blue!70!black, above=0.6cm of genjobs] (secuencias) {Secuencias\\de entrada};
    ...
    \node[channel, right=0.8cm of job2] (jobchan) {jobs\\Chan};
    ...
    \begin{scope}[on background layer]
        \node[groupbox=purple!70!black, fit=(w1)(w2)(w3)(wdots)(wn), label={[font=\bfseries\tiny, text=purple!70!black]above:Workers}] (workerpool) {};
    \end{scope}

    % ========== CANAL DE RESULTADOS (vertical) ==========
    \node[channel, right=0.8cm of w2] (resultchan) {result\\Chan};

    % ========== RECOLECCIÓN ==========
    \node[mainbox, fill=green!60!black, right=1cm of resultchan] (collect) {Recolectar\\resultados};
