...
Se realizaron benchmarks exhaustivos comparando ambas implementaciones con secuencias aleatorias de longitud creciente (20 a 260 caracteres). Los resultados revelaron hallazgos importantes:

\begin{itemize}
    \item La versión secuencial fue consistentemente más rápida en todo el rango evaluado, con un speedup promedio de 0.37x (la versión paralela resultó aproximadamente 2.7 veces más lenta en promedio).
    \item El componente de construcción de tabla DP mostró la mayor sobrecarga al crear y sincronizar goroutines, con speedup entre 0.01x y 0.04x, siendo aproximadamente 30-50 veces más lento que la versión secuencial.
    \item El backtracking paralelo mostró mejor rendimiento relativo que el llenado de la tabla. Para secuencias pequeñas (20-90 caracteres), el speedup fue de 0.26x-0.37x, pero mejoró progresivamente alcanzando 0.66x-0.74x para secuencias largas (180-260 caracteres), indicando una tendencia positiva con el tamaño del problema.
    \item El número de LCS encontradas crece exponencialmente, alcanzando 186,744 subsecuencias para secuencias de longitud 200 y 379,512 para longitud 260.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Análisis de escalabilidad}: No se estudió formalmente cómo escala el rendi-miento con el número de núcleos disponibles.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Estructuras de datos lock-free}: Explorar estructuras de datos concurrentes que reduzcan la contención en el registro de caminos visitados, como tablas hash lock-free o técnicas de versionado.
\end{enumerate}
